...
struct TimelineView: View {
    private enum TimelineNextActionState {
        case loading
        case resumeCapture
        case checkCapture
        case start
        ...
        RowSurface(tone: timelineNextActionTone, isHovering: false) {
            VStack(alignment: .leading, spacing: DesignSystem.Spacing.md) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ViewThatFits(in: .horizontal) {
                            HStack(alignment: .firstTextBaseline, spacing: DesignSystem.Spacing.xs) {
                                timelineNextActionEyebrow

                                Spacer(minLength: 0)

                                timelineNextActionStatusPill
                            }
                ...
                Button {
                    ...
                    timelineActionLabel(L(timelineNextActionButtonTitleKey), systemImage: timelineNextActionButtonIconName)
                        .frame(maxWidth: .infinity)
                }
                .buttonStyle(.borderedProminent)
                .controlSize(.small)
                .tint(DesignSystem.Colors.accentSkyBlue)
                .disabled(timelineNextActionState == .loading)
                .accessibilityIdentifier("timeline.next.primary")
            }
        }
        .accessibilityIdentifier("timeline.nextAction")
    }

    private var timelineNextActionEyebrow: some View {
    ...
    private var timelineNextActionStatusPill: some View {
        StatusPill(
            L(timelineNextActionButtonTitleKey),
            systemImage: timelineNextActionButtonIconName,
            tone: timelineNextActionTone
        )
        .fixedSize(horizontal: true, vertical: false)
        .accessibilityIdentifier("timeline.next.status")
    }
    ...
    private var timelineNextActionState: TimelineNextActionState {
        if isLoading {
            return .loading
        }
        if appState.trackingPaused {
        ...
        if appState.dateRangeMode == .day && !timelineDailyFolderReady {
            return .setFolder
        }
        return .closeout
    }
    ...
    private var timelineNextActionTitleKey: String {
        switch timelineNextActionState {
        case .loading:
    ...
    private var timelineNextActionButtonTitleKey: String {
        switch timelineNextActionState {
        case .loading:
    ...
    private var timelineNextActionIconName: String {
        ...
        case .checkCapture, .retryDailyLog:
            return "exclamationmark.triangle.fill"
        case .start, .addContext:
    ...
    private var timelineNextActionTone: DesignSystem.StatusTone {
        ...
        case .loading:
            return .info
        case .resumeCapture, .resetFilters, .cleanup, .addContext, .setFolder:
            return .warning
        case .checkCapture, .retryDailyLog:
            return .critical
        case .start:
    ...
    private var timelineDailyFolderReady: Bool {
